...
var helpTextPath = path.join(__dirname, '../src/lib/help.txt');
...
function createLookup(addressesByHostname) {
	return function lookup(hostname, options, callback) {
		...
		var addresses = addressesByHostname[String(hostname).toLowerCase()];
		if (!addresses) {
			var error = new Error('getaddrinfo ENOTFOUND ' + hostname);
...
var publicLookup = createLookup({
	'example.com': ['93.184.216.34'],
	'notallowed.example': ['93.184.216.35'],
});
...
describe('Basic functionality', function () {
	...
	it('POST plain text', function (done) {
		request(cors_anywhere)
			...
			.expect('Access-Control-Allow-Origin', '*')
			.expect(
				'{"this is a request body & should not be mangled":1.00}',
				done,
			);
	});
	...
	it('POST with 302 redirect should be followed', function (done) {
		request(cors_anywhere)
			...
			.expect(
				'x-cors-redirect-1',
				'302 http://example.com/redirectposttarget',
			)
	...
	it('GET with 302 redirect without Location header should not be followed', function (done) {
		...
		request(cors_anywhere)
			...
			.expect(
				'x-request-url',
				'http://example.com/redirectwithoutlocation',
			)
	...
	it('GET with 302 redirect to an invalid Location should not be followed', function (done) {
		...
		request(cors_anywhere)
			...
			.expect(
				'x-request-url',
				'http://example.com/redirectinvalidlocation',
			)
...
describe('Proxy errors', function () {
	...
	before(function () {
		...
		bad_http_server_url =
			'http://127.0.0.1:' + bad_http_server.listen(0).address().port;
	});
	after(function (done) {
	...
	before(function () {
		...
		bad_status_http_server_url =
			'http://127.0.0.1:' +
			bad_status_http_server.listen(0).address().port;
	});
	after(function (done) {
	...
	before(function () {
		...
		bad_tcp_server_url =
			'http://127.0.0.1:' + bad_tcp_server.listen(0).address().port;
	});
	after(function (done) {
	...
	it('Proxy error', function (done) {
		request(cors_anywhere)
			...
			.expect(
				404,
				'Not found because of proxy error: Error: throw node',
				done,
			);
	});
	...
	it('Content-Length mismatch', function (done) {
		...
		request(cors_anywhere)
			...
			.expect(
				404,
				'Not found because of proxy error: ' + errorMessage,
				done,
			);
	});
	...
	it('Invalid HTTP status code', function (done) {
		...
		} else if (parseInt(process.versions.node, 10) < 20) {
			errorMessage =
	...
	it('Content-Encoding invalid body', function (done) {
		request(cors_anywhere)
			...
			.expect(function (res) {
				...
				assert.match(
					res.text,
					/^Not found because of proxy error: Error: Parse Error:/,
				);
			})
			.end(done);
	});
...
describe('NODE_TLS_REJECT_UNAUTHORIZED', function () {
	...
	it('respects certificate errors by default', function (done) {
		...
		request(cors_anywhere)
			...
			.expect('Access-Control-Allow-Origin', '*')
			.expect(
				'Not found because of proxy error: ' + certErrorMessage,
				done,
			);
	});
...
describe('destinationWhitelist', function () {
	afterEach(stopServer);

	it('GET /http://example.com with permitted destination', function (done) {
		cors_anywhere = createServer({
	...
	it('GET /http://notallowed.example with denied destination', function (done) {
		cors_anywhere = createServer({
			destinationWhitelist: ['http://example.com'],
			destinationLookup: publicLookup,
		});
		...
		request(cors_anywhere)
			.get('/http://notallowed.example/')
			.expect('Access-Control-Allow-Origin', '*')
			.expect(403, 'Destination is not allowed by this proxy.', done);
	});

	it('GET /http://example.com with destination requiring https', function (done) {
		cors_anywhere = createServer({
			destinationWhitelist: ['https://example.com'],
			destinationLookup: publicLookup,
		});
		cors_anywhere_port = cors_anywhere.listen(0).address().port;
	...
	it('GET /https://example.com with destination requiring https', function (done) {
		cors_anywhere = createServer({
			destinationWhitelist: ['https://example.com'],
			destinationLookup: publicLookup,
		});
	...
	it('GET /example.com with no scheme and destination requiring https', function (done) {
		cors_anywhere = createServer({
			destinationWhitelist: ['https://example.com'],
			destinationLookup: publicLookup,
		});
	...
	it('GET /http://example.com:1234 with default-port destination', function (done) {
		cors_anywhere = createServer({
	...
	it('GET /http://127.0.0.1 rejects unsafe IPv4 literal', function (done) {
		cors_anywhere = createServer({
			destinationWhitelist: ['http://127.0.0.1'],
			destinationLookup: publicLookup,
		});
		cors_anywhere_port = cors_anywhere.listen(0).address().port;
	...
	it('GET /http://[::1] rejects unsafe IPv6 literal', function (done) {
		cors_anywhere = createServer({
			destinationWhitelist: ['http://[::1]'],
			destinationLookup: publicLookup,
		});
		cors_anywhere_port = cors_anywhere.listen(0).address().port;
	...
	it('GET /http://[fec0::1] rejects site-local IPv6 literal', function (done) {
		cors_anywhere = createServer({
			destinationWhitelist: ['http://[fec0::1]'],
			destinationLookup: publicLookup,
		});
		cors_anywhere_port = cors_anywhere.listen(0).address().port;
	...
	it('GET /http://[64:ff9b:1::a00:1] rejects local-use NAT64 IPv6 literal', function (done) {
		cors_anywhere = createServer({
			destinationWhitelist: ['http://[64:ff9b:1::a00:1]'],
			destinationLookup: publicLookup,
		});
		cors_anywhere_port = cors_anywhere.listen(0).address().port;

		request(cors_anywhere)
			.get('/http://[64:ff9b:1::a00:1]/')
			.expect('Access-Control-Allow-Origin', '*')
			.expect(
				403,
				'Destination address is not allowed by this proxy.',
	...
	it('GET /http://example.com rejects unsafe IPv6 DNS result', function (done) {
		cors_anywhere = createServer({
			destinationWhitelist: ['http://example.com'],
			destinationLookup: createLookup({
				'example.com': ['fec0::1'],
	...
	it('GET /http://example.com rejects local-use NAT64 IPv6 DNS result', function (done) {
		cors_anywhere = createServer({
			destinationWhitelist: ['http://example.com'],
			destinationLookup: createLookup({
	...
	it('GET with redirect to denied destination should be blocked', function (done) {
		...
		request(cors_anywhere)
			.get('/http://example.com/redirecttonotallowed')
			.redirects(0)
			.expect('Access-Control-Allow-Origin', '*')
			.expect('x-cors-redirect-1', '302 http://notallowed.example/')
			.expect(403, 'Destination is not allowed by this proxy.', done);
	});

	it('GET with redirect to unsafe destination should be blocked', function (done) {
		cors_anywhere = createServer({
			destinationWhitelist: ['http://example.com', 'http://127.0.0.1'],
			destinationLookup: publicLookup,
		});
		cors_anywhere_port = cors_anywhere.listen(0).address().port;

		request(cors_anywhere)
			.get('/http://example.com/redirecttoprivate')
			.redirects(0)
			.expect('Access-Control-Allow-Origin', '*')
			.expect('x-cors-redirect-1', '302 http://127.0.0.1/private')
			.expect(
				403,
				'Destination address is not allowed by this proxy.',
				done,
	...
	it('OPTIONS with permitted origin and destination', function (done) {
		cors_anywhere = createServer({
			originWhitelist: ['https://permitted.origin.test'],
			destinationWhitelist: ['http://example.com'],
			destinationLookup: publicLookup,
	...
	it('OPTIONS with no-scheme destination requiring https', function (done) {
		cors_anywhere = createServer({
			originWhitelist: ['https://permitted.origin.test'],
			destinationWhitelist: ['https://example.com'],
			destinationLookup: publicLookup,
	...
	it('OPTIONS with permitted origin and denied destination', function (done) {
		cors_anywhere = createServer({
			originWhitelist: ['https://permitted.origin.test'],
			destinationWhitelist: ['http://example.com'],
			destinationLookup: publicLookup,
		...
		request(cors_anywhere)
			.options('/http://notallowed.example/')
			.set('Origin', 'https://permitted.origin.test')
			.set('Access-Control-Request-Method', 'GET')
			.expect(
				'Access-Control-Allow-Origin',
	...
	it('OPTIONS with denied origin and permitted destination', function (done) {
		cors_anywhere = createServer({
			originWhitelist: ['https://permitted.origin.test'],
			destinationWhitelist: ['http://example.com'],
			destinationLookup: publicLookup,
		...
		request(cors_anywhere)
			.options('/http://example.com/')
			.set('Origin', 'https://denied.origin.test')
			.set('Access-Control-Request-Method', 'GET')
			.expectNoHeader('Access-Control-Allow-Origin')
			.expect(
				403,
				'The origin "https://denied.origin.test" was not allowed by the operator of this proxy.',
				done,
			);
	});
});
	...
	it('GET /example.com without header (requireHeader as string)', function (done) {
		stopServer(function () {
			...
			request(cors_anywhere)
				...
				.expect(
					400,
					'Missing required request header. Must specify one of: origin',
					done,
				);
		});
	});
...
describe('helpFile', function () {
	...
	it('GET / with custom text helpFile', function (done) {
		...
		var customHelpText = fs.readFileSync(customHelpTextPath, {
			encoding: 'utf8',
		});
	...
	it('GET / with non-existent help file', function (done) {
		var customHelpTextPath = path.join(
			__dirname,
